...
- Typed rooms. `client.joinOrCreate('my_room', stateType: MyRoomState.new)`
  is the Dart spelling of C#'s `JoinOrCreate<MyRoomState>("my_room")`:
  `room.state` is then the generated class, and `onStateChange` becomes a
  `Stream<TState>` carrying it — `room.onStateChange.first` is the bind-once
  idiom for registering schema callbacks. Join options moved to a named
  parameter (`options:`) to make space. Rooms joined without a `stateType:`
  keep the dynamic `SchemaInstance` accessors.
- Typed state access through `schema-codegen --dart`. The generator emits one
  class per schema, wrapping the same native handle the dynamic API uses:
  `room.stateAs(MyRoomState.new)` returns the typed root, collections come
  back as `MapSchema<Player>` / `ArraySchema<Item>`, and child refs as the
  generated class. The room caches wrappers per native instance and drops
  them on reconnect, so reaching state afresh every frame is the cheap path
  as well as the safe one. The runtime pieces (`SchemaRef`, `MapSchema<T>`,
  `ArraySchema<T>`) are hand-written in the package; codegen stays optional
  and the dynamic API keeps working.
- `Callbacks.get(room)` — schema callbacks on their own object, shaped
  after the C# SDK's `Callbacks.Get(room)` strategy. Registration takes the
  typed field to observe: `callbacks.onAdd(state.players, (key, player) ...)`
  — where C# names the field with an expression, Dart passes the generated
  getter's value, which records the field it came from, so the registration
  binds to the field and survives the collection being replaced. Collection
  handlers receive `(key, value)` (a `String` key for maps, `int` index for
  arrays), matching C#. `listen<V>` types the scalar handler, `listenRef`
  wraps child instances in their generated class, and `onAddByName` /
  `onRemoveByName` are the string-based variants for use without codegen.
  `listen` and `onAdd` take `immediate` (default true) to replay current
  state on registration.
- `onChange` on schema instances — fires after any property of the instance
  changes, via the core's `on_change_instance`.
- Client-side prediction, the 0.18 headline feature, with ordinary Dart
  closures for every step function:
  - `Predict.get(room)` with `attach` / `attachAll` taking the same declarative
...
- `room.listen` / `room.onAdd` / `room.onRemove` moved to `StateCallbacks`
  (see `Callbacks.get` above) and now take the instance to observe rather
  than a raw `instanceHandle:` integer.
- `SchemaMap` and `SchemaArray` `operator[]` are direct native lookups. The
  old path snapshotted the whole collection and scanned it, allocating a Dart
  string per entry, on every call.
