...
fn descriptorFor(name: []const u8) c.colyseus_quantized_descriptor_t {
    if (std.mem.eql(u8, name, "clamp8_0_10")) return c.colyseus_quantize_resolve(0, 10, 8, false);
    if (std.mem.eql(u8, name, "clamp16_pitch")) return c.colyseus_quantize_resolve(-1.5, 1.5, 16, false);
    if (std.mem.eql(u8, name, "clamp32_unit")) return c.colyseus_quantize_resolve(0, 1, 32, false);
    if (std.mem.eql(u8, name, "wrap16_angle")) return c.colyseus_quantize_resolve(0, TWO_PI, 16, true);
    if (std.mem.eql(u8, name, "wrap8_degrees")) return c.colyseus_quantize_resolve(0, 360, 8, true);
    if (std.mem.eql(u8, name, "wrap32_angle")) return c.colyseus_quantize_resolve(0, TWO_PI, 32, true);
    unreachable;
}
...
test "codec_vectors" {
    var rows: usize = 0;
    var lines = std.mem.tokenizeScalar(u8, CODEC_VECTORS, '\n');
    while (lines.next()) |line| {
        var cols = std.mem.tokenizeSequence(u8, line, ", ");
        const desc = descriptorFor(cols.next().?);
        const input = parseNumber(cols.next().?);
        const expected_q = std.fmt.parseInt(u32, cols.next().?, 10) catch unreachable;
        const expected_rt = parseNumber(cols.next().?);

        const q = c.colyseus_quantize(&desc, input);
        try testing.expectEqual(expected_q, q);
        // shortest-roundtrip decimal parses back to the identical float64
        try testing.expectEqual(expected_rt, c.colyseus_dequantize(&desc, q));
        rows += 1;
    }
    try testing.expectEqual(@as(usize, 132), rows);
}

test "reflection_and_quantized_state" {
    // 5.0 reflection handshake: quantized descriptors, childPrimitive
    // collection slots (no colon packing), schema refs — auto-detected
    // into a dynamic vtable
    const reflection = [_]u8{ 128, 1, 255, 1, 128, 0, 2, 128, 1, 15, 255, 2, 128, 0, 130, 3, 255, 3, 128, 0, 4, 128, 1, 6, 128, 2, 8, 128, 3, 10, 128, 4, 11, 128, 5, 12, 128, 6, 13, 128, 7, 14, 255, 4, 128, 163, 121, 97, 119, 129, 169, 113, 117, 97, 110, 116, 105, 122, 101, 100, 132, 5, 255, 5, 128, 0, 0, 0, 0, 0, 0, 0, 0, 129, 24, 45, 68, 84, 251, 33, 25, 64, 130, 16, 131, 1, 255, 6, 128, 165, 112, 105, 116, 99, 104, 129, 169, 113, 117, 97, 110, 116, 105, 122, 101, 100, 132, 7, 255, 7, 128, 0, 0, 0, 0, 0, 0, 248, 191, 129, 0, 0, 0, 0, 0, 0, 248, 63, 130, 8, 131, 0, 255, 8, 128, 167, 112, 114, 101, 99, 105, 115, 101, 129, 169, 113, 117, 97, 110, 116, 105, 122, 101, 100, 132, 9, 255, 9, 128, 0, 0, 0, 0, 0, 0, 0, 0, 129, 0, 0, 0, 0, 0, 0, 240, 63, 130, 32, 131, 0, 255, 10, 128, 164, 110, 117, 109, 115, 129, 165, 97, 114, 114, 97, 121, 130, 255, 131, 166, 110, 117, 109, 98, 101, 114, 255, 11, 128, 164, 116, 97, 103, 115, 129, 163, 109, 97, 112, 130, 255, 131, 166, 115, 116, 114, 105, 110, 103, 255, 12, 128, 165, 99, 104, 105, 108, 100, 129, 163, 114, 101, 102, 130, 1, 255, 13, 128, 165, 105, 116, 101, 109, 115, 129, 165, 97, 114, 114, 97, 121, 130, 1, 255, 14, 128, 165, 108, 97, 98, 101, 108, 129, 166, 115, 116, 114, 105, 110, 103, 255, 15, 128, 1, 130, 16, 255, 16, 128, 0, 17, 255, 17, 128, 161, 118, 129, 166, 110, 117, 109, 98, 101, 114 };

    const serializer = c.colyseus_schema_serializer_create(null).?;
    defer c.colyseus_schema_serializer_free(serializer);
    ...
    // childPrimitive: nums is array-of-number (the old builder hardcoded "string")
    const nums = c.colyseus_dynamic_schema_get_by_name(state, "nums").*.data.array;
    try testing.expectEqual(@as(c_int, 3), nums.*.count);
    const n1: *f64 = @ptrCast(@alignCast(c.colyseus_array_schema_get(nums, 1).?));
    try testing.expectEqual(@as(f64, 2.5), n1.*);

    const tags = c.colyseus_dynamic_schema_get_by_name(state, "tags").*.data.map;
    const tag_a: [*c]const u8 = @ptrCast(c.colyseus_map_schema_get(tags, "a").?);
    try testing.expectEqualStrings("x", std.mem.span(tag_a));

    const child = c.colyseus_dynamic_schema_get_by_name(state, "child").*.data.ref;
    try testing.expectEqual(@as(f64, 7), c.colyseus_dynamic_schema_get_by_name(child, "v").*.data.num);

    const items = c.colyseus_dynamic_schema_get_by_name(state, "items").*.data.array;
    try testing.expectEqual(@as(c_int, 2), items.*.count);

    // patch: yaw changes, pitch clamps to min, nums grows
    const patch = [_]u8{ 128, 250, 162, 129, 0, 255, 1, 128, 3, 4 };
    c.colyseus_schema_serializer_patch(serializer, &patch, patch.len, 0);
